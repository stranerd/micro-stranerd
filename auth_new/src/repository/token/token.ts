...
import { TokenInput,RefreshTokenModel, UserModel } from "../../application/domain";
import * as fs from 'fs';

const config = require("../../config.json");

// PRIVATE and PUBLIC key
var privateKEY  = fs.readFileSync(config.token.private_key_path, 'utf8');
var publicKEY  = fs.readFileSync(config.token.public_key_path, 'utf8');
...
export class Token implements ITokenHandler {
    ...
    async generateJwtToken(payload: TokenInput): Promise<string> {
        ...
        const token = jwt.sign(payload, privateKEY, {expiresIn: config.token.access_token_lifetime,algorithm: config.token.algorithm});
        if (token) {
