...
export class EventMapper extends BaseMapper<EventFromModel, EventToModel, EventEntity> {
	mapFrom (model: EventFromModel | null) {
		...
		const { _id, title, data, classId, user, users, taskIds, readAt, createdAt, updatedAt } = model
		return new EventEntity({
			id: _id.toString(), title, data, classId, user, users, taskIds, readAt, createdAt, updatedAt
		})
	}
