...
export const QuestionChangeStreamCallbacks: ChangeStreamCallbacks<QuestionFromModel, QuestionEntity> = {
	created: async ({ after }) => {
		await appInstance.socketEmitter.emitCreated('questions/questions', after)
		await appInstance.socketEmitter.emitCreated(`questions/questions/${after.id}`, after)
	...
	updated: async ({ before, after, changes }) => {
		await appInstance.socketEmitter.emitUpdated('questions/questions', after)
		await appInstance.socketEmitter.emitUpdated(`questions/questions/${after.id}`, after)
	...
	deleted: async ({ before }) => {
		await appInstance.socketEmitter.emitDeleted('questions/questions', before)
		await appInstance.socketEmitter.emitDeleted(`questions/questions/${before.id}`, before)
